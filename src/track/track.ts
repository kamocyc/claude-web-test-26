...
/**
 * 路盤と地面の高さの差がこれ以下なら、**地形を切り盛りして路盤に合わせる**（m）。
 *
 * これを超えて地面が高いところは切土が深すぎるので敷けず（`'buried'`）、
 * 逆に低いところは盛らずに橋脚で渡す。つまり
 * 「ちょっとした凸凹は自動でならす、谷は橋、山は掘ってから」という切り分けになる。
 */
export const GRADE_TOL = 2
